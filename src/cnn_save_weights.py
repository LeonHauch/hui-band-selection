...
if __name__ == "__main__":
    ...
    with open("data/cnn_architecture.json") as f:
        arch = json.load(f)
    print(f"Retraining chosen architecture: kernel_size={arch['kernel_size']}, filters={arch['filters']}")

    model, val_f1, n_epochs = train_cnn(
        X_train, y_train_0, X_val, y_val_0, n_classes,
        kernel_size=arch["kernel_size"], filters=arch["filters"],
    )
    print(f"\nFinal val macro F1: {val_f1:.4f} (stopped at epoch {n_epochs})")

    torch.save(model.state_dict(), "data/cnn_model_all_bands.pt")
    print("Saved data/cnn_model_all_bands.pt")
